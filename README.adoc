...
using `git`, so the basic way to build the 42ITy(TM) ecosystem is to:
...
To update sources and rebuild whatever has changed later on, you can use e.g.
`make world`, or call a number of actions on specific components using a
