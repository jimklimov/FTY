...
#      esac
      ...
      if [ "$CI_PARMAKE_QUIET" = "yes" ]; then
        ...
        $CI_TIME make VERBOSE=0 V=0 BIOS_LOG_LEVEL=LOG_INFO -k -j4 "$BUILD_TGT" & 1>/dev/null
        PID_MAKE=$!
      else
        $CI_TIME make VERBOSE=0 V=0 BIOS_LOG_LEVEL=LOG_INFO -k -j4 "$BUILD_TGT" &
        PID_MAKE=$!
      fi
      ( minutes=0
        ticks=0
    ...
    ( RES=$?
      ...
      $CI_TIME make V=1 VERBOSE=1 BIOS_LOG_LEVEL=LOG_DEBUG "$BUILD_TGT" &
      PID_MAKE=$!
      ( minutes=0
        ticks=0
        limit=29
